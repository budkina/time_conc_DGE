...
clustering <- function(ma, metadata, id_to_gene, is_top, type, method, time)
{
	...
	ggsave(paste0(type,'PMA_', method, '_conc_', time, '_linear', suffix, '.png'),  plot = res$plot,  device = "png", width = 30,  height = 20,  units = "cm")
	save(res, file = paste0(type, 'PMA_', method, '_conc_', time, '_linear', suffix, '.RData'))
	degPlotCluster(res$normalized, time = "condition")
	dev.off()
	file.rename('Rplots.pdf', paste0(type,'PMA_', method, '_conc_', time, '_linear', suffix, '.pdf'))
	clusters <- res$df 
	...
	write.table(clusters, paste0(type, 'PMA_', method, '_conc_', time, '_clusters_linear', suffix, '.csv'), sep = '\t',  quote = F,  row.names = F)
	...
	for (cluster in cluster_ids)
	{
		...
		ggsave(paste0("const_time_sep_cluster_pictures/", type, 'PMA_', method, '_conc_', time, '_linear_', cluster, '.png'),
			plot = res$plot + ggtitle(paste0(type,'PMA, ', method, ', linear, time = ', time, ' cluster ', cluster)),
...
for (i in 1:5)
{
	...
	write.table(newdata, paste0(type,'PMA_DESeq2_conc_',time,'_linear.csv'), sep = '\t',  quote = F,  row.names = F)
	print(dim(newdata))
	...
	write.table(newdata, paste0(type,'PMA_edgeR_conc_',time,'_linear.csv'), sep = '\t',  quote = F,  row.names = F)
	print(dim(newdata))
	...
	newdata$geneName <- id_to_gene[rownames(newdata), ]$V2
	newdata <- newdata[,c(8,7,1,2,3,4,5,6)]
	write.table(newdata, paste0(type,'PMA_voom_conc_',time,'_linear.csv'), sep = '\t',  quote = F,  row.names = F)
	print(dim(newdata))
...
de_num_filename <- paste0(type,'PMA_conc_linear_linear_de_num.csv')
