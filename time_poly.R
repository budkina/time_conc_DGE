...
clustering <- function(ma, metadata, id_to_gene, is_top, type, method, conc)
{
	...
	ggsave(paste0(type,'PMA_', method, '_time_', conc, '_poly', suffix, '.png'),  plot = res$plot,  device = "png", width = 30,  height = 20,  units = "cm")
	save(res, file = paste0(type, 'PMA_', method, '_time_', conc, '_poly', suffix, '.RData'))
	degPlotCluster(res$normalized, time = "condition")
	dev.off()
	file.rename('Rplots.pdf', paste0(type,'PMA_', method, '_time_', conc, '_poly', suffix, '.pdf'))
	clusters <- res$df 
	...
	write.table(clusters, paste0(type, 'PMA_', method, '_time_', conc, '_clusters_poly', suffix, '.csv'), sep = '\t',  quote = F,  row.names = F)
	...
	for (cluster in cluster_ids)
	{
		...
		ggsave(paste0("const_conc_sep_cluster_pictures/", type, 'PMA_', method, '_time_', conc, '_poly_', cluster, '.png'),
			plot = res$plot + ggtitle(paste0(type,'PMA, ', method, ', poly, conc = ', conc, ' cluster ', cluster)),
...
for (i in 1:5)
{
	...
	write.table(newdata, paste0(type,'PMA_edgeR_time_',conc,'.csv'), sep = '\t',  quote = F,  row.names = F)
	print(dim(newdata))
	...
	newdata$geneName <- id_to_gene[rownames(newdata), ]$V2
	newdata <- newdata[,c(9,8,1,2,3,4,5,6,7)]
	write.table(newdata, paste0(type,'PMA_voom_time_',conc,'.csv'), sep = '\t',  quote = F,  row.names = F)
	print(dim(newdata))
...
de_num_filename <- paste0(type,'PMA_time_poly_poly_de_num.csv')
