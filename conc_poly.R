...
clustering <- function(ma, metadata, id_to_gene, is_top, type, method, time)
{
	...
	ggsave(paste0(type,'PMA_', method, '_conc_', time, '_poly', suffix, '.png'),  plot = res$plot,  device = "png", width = 30,  height = 20,  units = "cm")
	save(res, file = paste0(type, 'PMA_', method, '_conc_', time, '_poly', suffix, '.RData'))
	degPlotCluster(res$normalized, time = "condition")
	dev.off()
	file.rename('Rplots.pdf', paste0(type,'PMA_', method, '_conc_', time, '_poly', suffix, '.pdf'))
	clusters <- res$df 
	...
	write.table(clusters, paste0(type, 'PMA_', method, '_conc_', time, '_clusters_poly', suffix, '.csv'), sep = '\t',  quote = F,  row.names = F)
	...
	for (cluster in cluster_ids)
	{
		...
		ggsave(paste0("const_time_sep_cluster_pictures/", type, 'PMA_', method, '_conc_', time, '_poly_', cluster, '.png'),
			plot = res$plot + ggtitle(paste0(type,'PMA, ', method, ', poly, time = ', time, ' cluster ', cluster)),
...
for (i in 1:5)
{
	...
	write.table(newdata, paste0(type,'PMA_DESeq2_conc_',time,'.csv'), sep = '\t',  quote = F,  row.names = F)
	print(dim(newdata))
	...
	write.table(newdata, paste0(type,'PMA_edgeR_conc_',time,'.csv'), sep = '\t',  quote = F,  row.names = F)
	print(dim(newdata))
	...
	write.table(newdata, paste0(type,'PMA_voom_conc_',time,'.csv'), sep = '\t',  quote = F,  row.names = F)
	print(dim(newdata))
...
de_num_filename <- paste0(type,'PMA_conc_poly_poly_de_num.csv')
